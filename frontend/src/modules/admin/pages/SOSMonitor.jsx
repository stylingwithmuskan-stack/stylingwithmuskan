                                ...
                                <CardContent className="p-4 flex flex-col md:flex-row md:items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <Badge className="bg-red-500/15 text-red-500 border-red-500/30 text-[8px] font-black uppercase">{a.userType || "SOS"}</Badge>
                                            <Badge className="bg-orange-500/15 text-orange-500 border-orange-500/30 text-[8px] font-black uppercase">{a.message || "Emergency Alert"}</Badge>
                                            <span className="text-[10px] text-muted-foreground">#{String(a._id).slice(-6).toUpperCase()}</span>
                                        </div>
                                    ...
                                    <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                                        ...
                                        <Button 
                                            className="h-9 bg-red-600 hover:bg-red-700 rounded-xl font-bold gap-1"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                const phone = a.userPhone || a.phone;
                                                if (phone) {
                                        ...
                                        <Button variant="outline" className="h-9 rounded-xl font-bold gap-1 hover:bg-green-500/10 hover:text-green-500 hover:border-green-500/30" onClick={() => { resolveSOSAlert(a._id); load(); }}><CheckCircle className="h-3.5 w-3.5" /> Resolve</Button>
                                    </div>
                                </CardContent>
                            </Card>
                        </motion.div>
            ...
            {resolved.length > 0 && (
                <Card className="border-border/50 shadow-none">
                    ...
                    <CardContent className="space-y-2">
                        {resolved.map(a => (
                            <div key={a._id} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground">{a.createdAt ? new Date(a.createdAt).toLocaleString() : "N/A"}</p>
                                    </div>
                                </div>
