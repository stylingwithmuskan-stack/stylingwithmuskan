...
export default function CustomEnquiries() {
  ...
  const beginQuote = (enq) => {
    ...
    const itemsToQuote = enq.quote?.items?.length ? enq.quote.items : (enq.items || []);
            ...
            <CardHeader>
              <CardTitle className="text-base font-bold flex items-center justify-between">
                ...
                <div className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-lg flex items-center gap-1 ${
                  ...
                  enq.status === "final_approved" ? "bg-blue-500/10 text-blue-500 border border-blue-500/20" :
                  enq.status === "rejected" ? "bg-red-500/10 text-red-400 border border-red-400/20" :
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button onClick={closeModal} className="w-8 h-8 rounded-full bg-muted flex items-center justify-center transition-colors hover:bg-muted/80">
                <X className="h-4 w-4" />
              </button>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <Input
                          ...
                          className="h-8 pl-7 text-[11px] font-black rounded-lg bg-secondary/40 border-border/50 text-foreground"
                        />
                      </div>
            ...
            <div className="flex gap-2 pt-2">
              {["admin_approved", "waiting_for_customer_payment", "advance_paid"].includes(selectedEnq.status) ? (
              ...
              <Button variant="outline" onClick={closeModal} className="h-11 rounded-xl font-bold">Close</Button>
            </div>
          </motion.div>
        </div>,
        document.body
