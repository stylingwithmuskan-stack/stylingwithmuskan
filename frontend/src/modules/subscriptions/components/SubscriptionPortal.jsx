    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {plans.map((plan) => {
          ...
          return (
            <motion.div key={plan.planId} whileHover={{ y: -2 }}>
              <Card className={`h-full border ${isCurrentPlan ? "border-primary shadow-lg shadow-primary/10" : "border-border/60 shadow-sm"}`}>
                <CardHeader>
                  ...
                  <CardDescription>{plan.tagline || "Configured by admin panel"}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <p className="text-3xl font-black">INR {Number(plan.price || 0).toLocaleString()}</p>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mt-1">{plan.billingCycle || "Custom plan"}</p>
                  </div>
