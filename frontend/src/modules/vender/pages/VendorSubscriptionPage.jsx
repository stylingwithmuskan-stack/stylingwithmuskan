...
const VendorSubscriptionPage = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        setPlans(response.plans.filter(p => p.userType === 'vendor'));
      } catch (error) {
  ...
  const handleSubscribe = async (planId) => {
    try {
      ...
      const userId = 'vendor123'; 
      const response = await api.subscriptions.subscribe({ planId, userId, userType: 'vendor' });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8">SWM City Manager Enterprise</h1>
