...
const ProviderSubscriptionPage = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        setPlans(response.plans.filter(p => p.userType === 'provider'));
      } catch (error) {
  ...
  const handleSubscribe = async (planId) => {
    try {
      ...
      const userId = 'provider123'; 
      const response = await api.subscriptions.subscribe({ planId, userId, userType: 'provider' });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8">SWM Pro Partner Subscription</h1>
