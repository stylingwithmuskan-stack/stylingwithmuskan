...
import { ChevronLeft, Headphones, MessageSquare, Phone, Send, CheckCircle2 } from "lucide-react";
...
export default function SWMSupport() {
    ...
    // Sample chat messages for UI
    const chatMessages = [
        { id: 1, text: "Hello! How can we help you today?", time: "10:30 AM" },
        { id: 2, text: "Our support team is available 24/7 to assist you.", time: "10:31 AM" },
    ];
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-50 -m-4 md:m-0">
            ...
            <div className="p-4 grid grid-cols-2 gap-3 bg-slate-50">
                <button 
                    onClick={() => handleQuickAction("I need help with a payment issue")}
                    className="bg-white p-4 rounded-2xl border border-slate-200 text-left hover:border-purple-300 transition-all shadow-sm group"
                >
                ...
                <button 
                    onClick={() => handleQuickAction("I need support with an active job")}
                    className="bg-white p-4 rounded-2xl border border-slate-200 text-left hover:border-purple-300 transition-all shadow-sm group"
                >
